...
public:
  ...
  // Matt's change start
  double GetValue(double rowKey, double colKey, double TableKey, double FrameKey) const;
  // Matt's change end
  /** Read the table in.
...
private:
  ...
  enum type {tt1D, tt2D, tt3D, tt4D} Type;
  //enum axis {eRow=0, eColumn, eTable};
  enum axis {eRow=0, eColumn, eTable, eFrame};
  bool internal;
  //FGPropertyManager *lookupProperty[3];
  FGPropertyManager *lookupProperty[4];
  FGPropertyManager * interpolation_level_property;
  mutable int interpolation_level;
  mutable BSpline1D bs1;
  mutable BSpline2D bs2;
  mutable vector < Matt::BSplineD > bs;
  void Data2BSplines();
  // Matt's change end
  double** Data;
